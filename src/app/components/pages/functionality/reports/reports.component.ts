...
export class ReportsComponent implements OnInit {
  ...
  // Modificar el método onReportSaved para corregir la detección de edición vs creación
  onReportSaved(report: ReportWithWorkshopsDto): void {
    // Determinar si es una creación o una edición de manera más precisa
    const isNewReport = (() => {
  ...
  // Mejorar el método logReportActivity para asegurar que extrae correctamente la información del reporte
  private logReportActivity(action: string, reportData: ReportDto | ReportWithWorkshopsDto): void {
    ...
    this.authService.getLoggedUserInfo().subscribe({
      next: (currentUser) => {
        // Extraer la información del reporte de manera más segura
        const report = "report" in reportData ? reportData.report : reportData

        // Crear objeto de actividad con información más detallada
        const activity = {
